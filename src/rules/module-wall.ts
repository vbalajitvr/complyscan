...
export const remoteModuleWallRule: ScanRule = {
  ...
  run(files: ParsedFile[], _context: ScanContext): Finding[] {
    ...
    // in the Article 12 universe but are not within complyscan's scope, so
    ...
    return candidates.map(({ name, source, matchedTokens, matchedInputKeys, filePath }) => {
      ...
      return {
        ...
        remediation:
          'Run complyscan against `terraform show -json plan.json` for full apply-time resolution, or define Bedrock logging in the root module where it can be scanned.',
